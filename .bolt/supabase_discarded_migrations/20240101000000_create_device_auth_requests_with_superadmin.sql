...
-- Admins and Superadmins can see all requests
-- This assumes you have a 'users' table with a 'role' column.
CREATE POLICY "Allow admins and superadmins to view all requests"
  ON public.device_authorization_requests FOR SELECT
  USING (
    EXISTS (
      SELECT 1
      FROM public.users -- Corrected table name
...
-- Admins and Superadmins can update requests (to approve/deny and add notes)
CREATE POLICY "Allow admins and superadmins to update requests"
...
-- Admins and Superadmins can delete requests (optional, consider if this is needed or if status should be 'archived')
CREATE POLICY "Allow admins and superadmins to delete requests"
